...
class CoursesListPage(BasePage):
    def __init__(self, page: Page):
        ...
        self.courses_list_button = page.get_by_test_id('courses-list-toolbar-create-course-button')
        ...
        self.course_edit_menu_button = page.get_by_test_id('course-view-edit-menu-item-icon')
    ...
    def check_visible_create_course_button(self):
        expect(self.courses_list_button).to_be_visible()

    def click_visible_create_course_button(self):
        self.courses_list_button.click()

    def check_visible_courses_card(self, index: int, title: str, max_score: int, min_score: int, estimated_time: int):
        ...
        expect(self.course_title.nth(index)).to_have_text(f'Max score: {max_score}')

        expect(self.course_min_text.nth(index)).to_be_visible()
        expect(self.course_title.nth(index)).to_have_text(f'Min score: {min_score}')

        expect(self.course_estimated_time_text.nth(index)).to_be_visible()
        expect(self.course_title.nth(index)).to_have_text(f'Estimated time:  {estimated_time}')

    def check_edit_course(self, index: int):
    ...
    def check_delete_course(self, index: int):
        self.course_menu_button.nth(index).click()
